...
export default React.memo(function FilePicker({onFilesChange, progress, value: files, MAX_FILES, MIN_FILES}: FileUploadProps) {  
  ...
  return (
    <div className="flex flex-col justify-center w-full" >         
      ...
      >        
        <CloudUpload />        
        <span
          className={`${files.length < MIN_FILES ? "text-red-500 text-xs" : "text-gray-600 dark:text-gray-300"}`}
        >
          Drop files {files.length < MIN_FILES
          ? `Min. ${MIN_FILES} (${files.length}/${MIN_FILES})`
